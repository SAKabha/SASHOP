...
// ==========================================
// 1. تحميل الصورة من الرابط المباشر (SA SHOP)
// ==========================================
...
%new
- (void)openDevChannel {
    NSURL *url = [NSURL URLWithString:@"https://t.me/SAShop8"];
...
%new
- (void)openDevAccount {
    NSURL *url = [NSURL URLWithString:@"https://t.me/SAKabhaa"];
    if ([[UIApplication sharedApplication] canOpenURL:url]) {
        [[UIApplication sharedApplication] openURL:url options:@{} completionHandler:nil];
    }
}
...
// ==========================================
// 3. تعديل النصوص (UILabel) لتصبح تابعة لـ SA SHOP
// ==========================================
%hook UILabel
...
- (void)setText:(NSString *)text {
    ...
    if ([text isEqualToString:@"SA Shop"] || self.tag == 11 || [text containsString:@"SA Shop"]) {
        %orig(@"تطوير SA SHOP");
        return;
    }
    
    if (self.tag == 12 || [text containsString:@"8 ball pool mod"]) {
        %orig(@"SA SHOP - 8 Ball Pool");
        return;
    }
